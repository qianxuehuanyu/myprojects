...
(function () {
    var CAROUSEL_WIDTH = 800;//轮播图的宽度
    var CAROUSEL_HEIGHT = 600;//轮播图的高度
    var SWITCH_IMAGE_ANIMATION_DURATION = 300;//切换图片的动画效果的播放时长
    var carouselContent = document.querySelector("#carousel .carousel-content");//容纳轮播图片Div的容器
    var imageIndex = 0;//图片在数组中的位置
    var switchImageTimerId = -1;//用于切换图片的计时器
    var switchImageAnimationPlaying = false;//该变量用于指示当前是否正在切换图片
    ...
    function moveTo(target, fromX, toX, fromY, toY, duration, completeHandler) {
        ...
        var frameIndex = 0;
        var x = fromX, y = fromY;//fromX 开始left位置,  fromY 开始点的top位置
        var speedX = (toX - fromX) / frames; //toX 结束点的left位置, fromX 开始left位置
        var speedY = (toY - fromY) / frames; //toY 结束点的top位置, fromY 开始点的置top位置
